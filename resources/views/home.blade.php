...
<div class="container_home">
        ...
        <form action="{{action('RegistrosController@store')}}" method="POST" enctype="multipart/form-data">
          {{csrf_field()}}    
...
@if($contador>0)
    ...
    @foreach($presentes as $membro)

            <div class="card p-2">
                <div class="ribbon ribbon-top-left"><span>{{ $membro->cargo ?? '' }}</span></div>
                <div class="d-flex align-items-center dismay">

                    <div class="image">
                        <img width="155" class="membro-img" src="data:image/png;base64, {{ $membro->foto ?? '' }}">
                    </div>

                <div w-100 class="sheesh" style="height:60px">
                        
                    <p class="mb-0 mt-0" style="font-size: 20px"><b>{{ $membro->nome ?? '' }}</b></p>
                    <p class="mb-0 mt-0">{{ $membro->coordenadoria ?? '' }}</p><br>

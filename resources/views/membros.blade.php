    ...
    <div class="modal fade " id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true" >
        <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable " >
            ...
            <div class="modal-body">
                ...
                <form action="{{action([App\Http\Controllers\MembrosController::class, 'store'])}}" method="POST"  enctype="multipart/form-data">
                    {{csrf_field()}}
                    ...
                    <div class="form-group">
                        ...
                        <div class="row">
                            ...
                            <select name="horario[{{ $i }}][key]" class="form-control" required>
                                <option label=" "></option>
                            ...
                            <div class="col-md-4" style="padding-bottom:5px">
                                <select name="horario[{{ $i }}][value]" class="form-control" required>
                                    <option label=" "></option>
        ...
        <tbody>
            ...
            @else
                @foreach($membros as $membro)
                    <tr>
                        <td>
                            <img height="100px" width="100px" class="membro-img" src="data:image/png;base64, {{ $membro->foto ?? '' }}">
                        </td>
                        <td>
                            <b>{{ $membro->nome ?? '' }}</b>
                        </td>
                        ...
                        <td>
                            ...
                            @foreach ($membro->horario as $horario)
                                @if ($horario['key'] != null and $horario['value'] != null)
                                    <b>{{ $horario['key'] }}</b>: {{ $horario['value'] }}<br/>
                                @endif
                            @endforeach">Ver Horários</a>
                        </td> 
                        <td>
                            <div style="display:flex; justify-content: space-around;">
                                <button class="btn" style="background-color: #02b04d; color: white" title="Editar" data-toggle="modal" data-target="#editarModal{{$membro->id}}" style="margin-right:5px">
                                        <i class="fa fa-pencil-alt icone"></i>
                                </button>

                                <form action="{{action([App\Http\Controllers\MembrosController::class, 'destroy'], $membro->id)}}" method="POST"  enctype="multipart/form-data">
                                    {{csrf_field()}}
                                    {{method_field('DELETE')}}  
                        ...
                        <!-- Modal de Edição -->
                            <div class="modal fade " id="editarModal{{$membro->id}}" tabindex="-1" aria-labelledby="editarModalLabel" aria-hidden="true">
                                <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable ">
                                    <div class="modal-content animated-background" >
                                    <div class="modal-header">
                                    ...
                                    <div class="modal-body">
                                        ...
                                        <form action="{{action([App\Http\Controllers\MembrosController::class, 'update'], $membro->id)}}" method="POST"  enctype="multipart/form-data">
                                                                {{csrf_field()}}
                                                                {{method_field('PUT')}}
                                            {{csrf_field()}}
                                            <div class="form-group">
                                                <label for="nome" style="color:#fff">Nome</label>
                                                <input type="text"  value="{{$membro->nome}}" name="nome" class="form-control">
                                            </div>
                                            <div class="form-group">
                                                <label for="cargo" style="color:#fff">Cargo</label>
                                                <input type="text" value="{{$membro->cargo}}" name="cargo" class="form-control">
                                            </div>
                                            <div class="form-group">
                                                <div class="input-group mb-3">
                                                    <div class="custom-file">
                                                        ...
                                                        <input type="file" name="foto" id="foto" value="{{$membro->foto}}" onchange="previewFile()" accept="image/*" required>
                                                    </div>
                                                    <div class="input-group-append">
                                                        <span class="input-group-text" id="inputGroupFileAddon02">Upload</span>
                                                    </div>
                                            ...
                                            <div class="form-group">
                                                <label for="coordenadoria" style="color:#fff">Coordenadoria/Diretoria</label>
                                                <input type="text" value="{{$membro->coordenadoria}}" name="coordenadoria" class="form-control">
                                            </div>
                                            <div class="form-group">
                                                <label for="horario" style="color:#fff" >Horário de Sede</label>
                                                <div class="row">
                                                ...
                                                <div class="row">
                                                    <div class="col-md-6" style="padding-bottom:5px">
                                                    <select name="horario[{{ $i }}][key]" class="form-control">
                                                        <option label=" "></option>
                                                        <option value="Segunda-Feira">Segunda-Feira</option>
                                                        <option value="Terça-Feira">Terça-Feira</option>
                                                        <option value="Quarta-Feira">Quarta-Feira</option>
                                                    ...
                                                    <div class="col-md-4" style="padding-bottom:5px">
                                                        <select name="horario[{{ $i }}][value]" class="form-control">
                                                            <option label=" "></option>
                                                            <option value="8:00">8:00</option>
                                                            <option value="8:50">8:50</option>
                                                            <option value="9:40">9:40</option>
